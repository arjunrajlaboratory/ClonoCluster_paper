...
pll <- lapply(sn_v, function(sn){
  ...
  fn <- paste("../Figs/top_markers_box/", sn, "_top_markers_boxplot.pdf", sep = "")
...
vl <- lapply(c(1:10), function(r){
  ...
  venn.plot <- VennDiagram::venn.diagram(list(low, barc, mid, high), filename = NULL,
    ...
    fn <- paste("../Figs/Venn/", sn, "venn.pdf",sep = "")
...
ggsave(plot = plot, "../Figs/Venn/Venn_diagrams.pdf", width = 3, height = 2.5 * length(vl))
